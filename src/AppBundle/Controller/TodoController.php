...
class TodoController extends Controller
{
    ...
    public function listAction()
    {
        $todos = $this->getDoctrine()->getRepository('AppBundle:Todo')
                ->findBy(array('ended'=>'no'));
        $todosEndedquery = $this->getDoctrine()->getRepository('AppBundle:Todo')
                ->findBy(array('ended'=>'yes'));

        return $this->render('todo/index.html.twig',array('todos' => $todos,
                                                        'todosEndedquery'=>$todosEndedquery));
    }
    ...
    public function createAction(Request $request)
    {
        ...
        $form = $this->createFormBuilder($todo)
                ...
                ->add('due_date',DateType::class,array('widget'=>'single_text', 'attr'=> array('class' => 'formcontrol','style' =>'margin-bottom:15px','html5' => true)))
    ...
    public function deleteAction($id)
    {
            ...
            $this->addFlash('notice','todo '.$name.' removed');

            return $this->redirectToRoute('todo_list');
    }

    /**
     * @Route("/todo/ended/{id}", name="todo_ended")
     */
    public function endedAction($id)
    {
            $em = $this->getDoctrine()->getManager();
            $todo = $em->getRepository('AppBundle:Todo')->find($id);
            $todo->setEnded('yes');
            ...
            $this->addFlash('notice','todo ended');
